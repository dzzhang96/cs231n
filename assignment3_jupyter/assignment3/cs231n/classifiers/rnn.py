...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        """
        N = features.shape[0]
        captions = self._null * np.ones((N, max_length), dtype=np.int32)#先全设置成null
        ...
        for t in range(max_length):
            ...
            if self.cell_type == 'rnn':
                next_h, rnn_step_cache = rnn_step_forward(x, prev_h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                next_h, next_c,lstm_step_cache = lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b)
                prev_c = next_c
            else:
                raise ValueError('Invalid cell_type "%s"' % self.cell_type)
            
            #下面一步不用temporal_affine_forward()是因为此时在循环中，输入的参数为rnn_step_forward后的(N,H)不是(N,T,H)
            temporal_affine_out, temporal_affine_cache = affine_forward(next_h,W_vocab,b_vocab)
            captions[:,t] = np.argmax(temporal_affine_out,axis=1)#(N,M)
